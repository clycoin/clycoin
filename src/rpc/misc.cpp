...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total clycoin balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized clycoin balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "clycoin|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "clycoin")
            + HelpExampleRpc("debug", "clycoin,net")
        );
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"clycoinaddress\"\n"
            "\nReturn information about the given clycoin address.\n"
            "\nArguments:\n"
            "1. \"clycoinaddress\"     (string, required) The clycoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"clycoinaddress\", (string) The clycoin address validated\n"
...
CScript _createmultisig_redeemScript(const UniValue& params)
{
...
#ifdef ENABLE_WALLET
        // Case 1: CLYCoin address and we have full public key:
    ...
    if (result.size() > MAX_SCRIPT_ELECLYT_SIZE)
        throw runtime_error(
                strprintf("redeemScript exceeds size limit: %d > %d", result.size(), MAX_SCRIPT_ELECLYT_SIZE));
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are clycoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) clycoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"clycoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"clycoinaddress\"  (string, required) The clycoin address to use for the signature.\n"
