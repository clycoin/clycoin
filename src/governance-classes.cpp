...
void CSuperblockManager::CreateSuperblock(CMutableTransaction& txNewRet, int nBlockHeight, std::vector<CTxOut>& voutSuperblockRet)
{
    ...
    for(int i = 0; i < pSuperblock->CountPayments(); i++) {
        ...
        if(pSuperblock->GetPayment(i, payment)) {
            ...
            // PRINT NICE LOG OUTPUT FOR SUPERBLOCK PAYCLYT

            CTxDestination address1;
            ExtractDestination(payment.script, address1);
            CBitcoinAddress address2(address1);

            // TODO: PRINT NICE N.N CLYCOIN OUTPUT
      ...
      vecPayments()
{
    ...
    // FIRST WE GET THE START EPOCH, THE DATE WHICH THE PAYCLYT SHALL OCCUR
    nEpochStart = obj["event_block_height"].get_int();

    // NEXT WE GET THE PAYCLYT INFORMATION AND RECONSTRUCT THE PAYCLYT VECTOR
...
void CSuperblock::ParsePaymentSchedule(std::string& strPaymentAddresses, std::string& strPaymentAmounts)
{
    ...
    // LOOP THROUGH THE ADDRESSES/AMOUNTS AND CREATE PAYCLYTS
    /*
    ...
    */
    ...
    for (int i = 0; i < (int)vecParsed1.size(); i++) {
        ...
        if (!address.IsValid()) {
            std::ostringstream ostr;
            ostr << "CSuperblock::ParsePaymentSchedule -- Invalid CLYCoin Address : " <<  vecParsed1[i];
            LogPrintf("%s\n", ostr.str());
            throw std::runtime_error(ostr.str());
        }
...
std::string CSuperblockManager::GetRequiredPaymentsString(int nBlockHeight)
{
    ...
    // LOOP THROUGH SUPERBLOCK PAYCLYTS, CONFIGURE OUTPUT STRING
