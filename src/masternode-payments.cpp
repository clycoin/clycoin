...
*
*   Why is this needed?
*   - In CLYCoin some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockPayeeValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
    ...
    if(nBlockHeight < consensusParams.nSuperblockStartBlock) {
        ...
        if(sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYCLYT_ENFORCECLYT)) {
...
void FillBlockPayments(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet, std::vector<CTxOut>& voutSuperblockRet)
{
    ...
    // FILL BLOCK PAYEE WITH MASTERNODE PAYCLYT OTHERWISE
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet)
{
    ...
    // GET MASTERNODE PAYCLYT VARIABLES SETUP
...
int CMasternodePayments::GetMinMasternodePaymentsProto() {
    return sporkManager.IsSporkActive(SPORK_10_MASTERNODE_PAY_UPDATED_NODES)
            ? MIN_MASTERNODE_PAYCLYT_PROTO_VERSION_2
            : MIN_MASTERNODE_PAYCLYT_PROTO_VERSION_1;
}
...
void CMasternodePayments::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // disable all CLYCoin specific functionality

    if (strCommand == NetMsgType::MASTERNODEPAYCLYTSYNC) { //Masternode Payments Request Sync
        ...
        if(netfulfilledman.HasFulfilledRequest(pfrom->addr, NetMsgType::MASTERNODEPAYCLYTSYNC)) {
            // Asking for the payments list multiple times in a short period of time is no good
            LogPrintf("MASTERNODEPAYCLYTSYNC -- peer already asked me for the list, peer=%d\n", pfrom->id);
            Misbehaving(pfrom->GetId(), 20);
            return;
        }
        netfulfilledman.AddFulfilledRequest(pfrom->addr, NetMsgType::MASTERNODEPAYCLYTSYNC);

        Sync(pfrom, connman);
        LogPrintf("MASTERNODEPAYCLYTSYNC -- Sent Masternode payment votes to peer %d\n", pfrom->id);

    } else if (strCommand == NetMsgType::MASTERNODEPAYCLYTVOTE) { // Masternode Payments Vote for the Winner
...

        {
            ...
            if(mapMasternodePaymentVotes.count(nHash)) {
                LogPrint("mnpayments", "MASTERNODEPAYCLYTVOTE -- hash=%s, nHeight=%d seen\n", nHash.ToString(), nCachedBlockHeight);
                return;
            }
        ...
        if(vote.nBlockHeight < nFirstBlock || vote.nBlockHeight > nCachedBlockHeight+20) {
            LogPrint("mnpayments", "MASTERNODEPAYCLYTVOTE -- vote out of range: nFirstBlock=%d, nBlockHeight=%d, nHeight=%d\n", nFirstBlock, vote.nBlockHeight, nCachedBlockHeight);
            return;
        }

        std::string strError = "";
        if(!vote.IsValid(pfrom, nCachedBlockHeight, strError, connman)) {
            LogPrint("mnpayments", "MASTERNODEPAYCLYTVOTE -- invalid message, error: %s\n", strError);
            return;
        }

        if(!CanVote(vote.vinMasternode.prevout, vote.nBlockHeight)) {
            LogPrintf("MASTERNODEPAYCLYTVOTE -- masternode already voted, masternode=%s\n", vote.vinMasternode.prevout.ToStringShort());
            return;
        }
        ...
        if(!mnodeman.GetMasternodeInfo(vote.vinMasternode.prevout, mnInfo)) {
            ...
            LogPrintf("MASTERNODEPAYCLYTVOTE -- masternode is missing %s\n", vote.vinMasternode.prevout.ToStringShort());
        ...
        if(!vote.CheckSignature(mnInfo.pubKeyMasternode, nCachedBlockHeight, nDos)) {
            if(nDos) {
                LogPrintf("MASTERNODEPAYCLYTVOTE -- ERROR: invalid signature\n");
                Misbehaving(pfrom->GetId(), nDos);
            } else {
                // only warn about anything non-critical (i.e. nDos == 0) in debug mode
                LogPrint("mnpayments", "MASTERNODEPAYCLYTVOTE -- WARNING: invalid signature\n");
            }
        ...
        LogPrint("mnpayments", "MASTERNODEPAYCLYTVOTE -- vote: address=%s, nBlockHeight=%d, nHeight=%d, prevout=%s, hash=%s new\n",
                    address2.ToString(), vote.nBlockHeight, nCachedBlockHeight, vote.vinMasternode.prevout.ToStringShort(), nHash.ToString());

        if(AddPaymentVote(vote)){
            vote.Relay(connman);
            masternodeSync.BumpAssetLastTime("MASTERNODEPAYCLYTVOTE");
        }
    }
}
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    //require at least MNPAYCLYTS_SIGNATURES_REQUIRED signatures
    ...
    // if we don't have at least MNPAYCLYTS_SIGNATURES_REQUIRED signatures on a payee, approve whichever is the longest chain
    if(nMaxSignatures < MNPAYCLYTS_SIGNATURES_REQUIRED) return true;

    BOOST_FOREACH(CMasternodePayee& payee, vecPayees) {
        if (payee.GetVoteCount() >= MNPAYCLYTS_SIGNATURES_REQUIRED) {
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f CLYCOIN\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
...
bool CMasternodePaymentVote::IsValid(CNode* pnode, int nValidationHeight, std::string& strError, CConnman& connman)
{
    ...
    if(nBlockHeight >= nValidationHeight) {
        ...
        nMinRequiredProtocol = MIN_MASTERNODE_PAYCLYT_PROTO_VERSION_1;
    }
    ...
    if(nRank > MNPAYCLYTS_SIGNATURES_TOTAL) {
        // It's common to have masternodes mistakenly think they are in the top 10
        // We don't want to print all of these messages in normal mode, debug mode should print though
        strError = strprintf("Masternode is not in the top %d (%d)", MNPAYCLYTS_SIGNATURES_TOTAL, nRank);
        // Only ban for new mnw which is out of bounds, for old mnw MN list itself might be way too much off
        if(nRank > MNPAYCLYTS_SIGNATURES_TOTAL*2 && nBlockHeight > nValidationHeight) {
            strError = strprintf("Masternode is not in the top %d (%d)", MNPAYCLYTS_SIGNATURES_TOTAL*2, nRank);
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight, CConnman& connman)
{
    ...
    if (nRank > MNPAYCLYTS_SIGNATURES_TOTAL) {
        LogPrint("mnpayments", "CMasternodePayments::ProcessBlock -- Masternode not in the top %d (%d)\n", MNPAYCLYTS_SIGNATURES_TOTAL, nRank);
        return false;
    }
...
void CMasternodePayments::CheckPreviousBlockVotes(int nPrevBlockHeight)
{
    ...
    for (int i = 0; i < MNPAYCLYTS_SIGNATURES_TOTAL && i < (int)mns.size(); i++) {
        auto mn = mns[i];
        CScript payee;
        bool found = false;
...
void CMasternodePaymentVote::Relay(CConnman& connman)
{
    ...
    if (!masternodeSync.IsWinnersListSynced()) return;
    CInv inv(MSG_MASTERNODE_PAYCLYT_VOTE, GetHash());
    // relay votes only strictly to new nodes until DIP0001 is locked in to avoid being banned by majority of (old) masternodes
    connman.RelayInv(inv, fDIP0001WasLockedIn ? mnpayments.GetMinMasternodePaymentsProto() : MIN_MASTERNODE_PAYCLYT_PROTO_VERSION_2);
}
        ...
        if(mapMasternodeBlocks.count(h)) {
            BOOST_FOREACH(CMasternodePayee& payee, mapMasternodeBlocks[h].vecPayees) {
                ...
                BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                    ...
                    pnode->PushInventory(CInv(MSG_MASTERNODE_PAYCLYT_VOTE, hash));
                    nInvCount++;
                }
            }
        }
...
void CMasternodePayments::RequestLowDataPaymentBlocks(CNode* pnode, CConnman& connman)
{
    ...
    while(nCachedBlockHeight - pindex->nHeight < nLimit) {
        if(!mapMasternodeBlocks.count(pindex->nHeight)) {
            ...
            vToFetch.push_back(CInv(MSG_MASTERNODE_PAYCLYT_BLOCK, pindex->GetBlockHash()));
    ...
    while(it != mapMasternodeBlocks.end()) {
        ...
        // A clear winner (MNPAYCLYTS_SIGNATURES_REQUIRED+ votes) was found
        // or no clear winner was found but there are at least avg number of votes
        if(fFound || nTotalVotes >= (MNPAYCLYTS_SIGNATURES_TOTAL + MNPAYCLYTS_SIGNATURES_REQUIRED)/2) {
        ...
        if(GetBlockHash(hash, it->first)) {
            vToFetch.push_back(CInv(MSG_MASTERNODE_PAYCLYT_BLOCK, hash));
        }
        // We should not violate GETDATA rules
        if(vToFetch.size() == MAX_INV_SZ) {
...
{
    float nAverageVotes = (MNPAYCLYTS_SIGNATURES_TOTAL + MNPAYCLYTS_SIGNATURES_REQUIRED) / 2;
