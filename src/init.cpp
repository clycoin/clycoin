...
#include "config/clycoin-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "clycoin (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all CLYCoin specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N CLYCOIN anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
    ...
    strUsage += HelpMessageOpt("-mempoolreplacement", strprintf(_("Enable transaction replacement in the memory pool (default: %u)"), DEFAULT_ENABLE_REPLACECLYT));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The CLY Coin Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that CLY Coin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CLY Coin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    fEnableReplacement = GetBoolArg("-mempoolreplacement", DEFAULT_ENABLE_REPLACECLYT);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CLY Coin is shutting down."));
...
#endif
    // Make sure only a single CLY Coin process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CLY Coin is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CLY Coin is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    {
        if (cmt != SanitizeString(cmt, SAFE_CHARS_UA_COMCLYT))
            return InitError(strprintf(_("User Agent comment (%s) contains unsafe characters."), cmt));
        uacomments.push_back(SanitizeString(cmt, SAFE_CHARS_UA_COMCLYT));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CLY Coin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart CLY Coin to complete") << "\n";
    ...
    // ********************************************************* Step 11c: update block tip in CLYCoin modules
    ...
    // ********************************************************* Step 11d: start clycoin-ps-<smth> threads
