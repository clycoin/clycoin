...
const char * const BITCOIN_CONF_FILENAME = "clycoin.conf";
const char * const BITCOIN_PID_FILENAME = "clycoind.pid";
 ...
 * by well-meaning clycoin adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "clycoin" is a composite category enabling all CLYCoin-related debug output
            if(ptrCategory->count(string("clycoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CLYCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CLYCoin
    // Mac: ~/Library/Application Support/CLYCoin
    // Unix: ~/.clycoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CLYCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CLYCoin";
#else
    // Unix
    return pathRet / ".clycoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty clycoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override clycoin.conf
