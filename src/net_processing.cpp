...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        CLYCoin Related Inventory Messages
    ...
    */
    ...
    case MSG_MASTERNODE_PAYCLYT_VOTE:
        return mnpayments.mapMasternodePaymentVotes.count(inv.hash);

    case MSG_MASTERNODE_PAYCLYT_BLOCK:
        {
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        LogPrint("net", "ProcessGetData -- inv = %s\n", inv.ToString());
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_MASTERNODE_PAYCLYT_VOTE) {
                    if(mnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnpayments.mapMasternodePaymentVotes[inv.hash];
                        connman.PushMessage(pfrom, NetMsgType::MASTERNODEPAYCLYTVOTE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_MASTERNODE_PAYCLYT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs))
        {
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                     ...
                     ++mi)
                {
                    ...
                    // resolution (that is, feeding clycoin an invalid transaction based on LegitTxX in order to get
    ...
    else if (strCommand == NetMsgType::FILTERADD)
    {
        ...
        if (vData.size() > MAX_SCRIPT_ELECLYT_SIZE)
        {
            LOCK(cs_main);
            Misbehaving(pfrom->GetId(), 100);
        } else {
