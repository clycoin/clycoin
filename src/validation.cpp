...
#if defined(NDEBUG)
# error "CLY Coin cannot be compiled without assertions."
#endif
...
bool fEnableReplacement = DEFAULT_ENABLE_REPLACECLYT;
...
{
    bool fDIP0001Active_context = (VersionBitsState(pindexPrev, Params().GetConsensus(), Consensus::DEPLOYCLYT_DIP0001, versionbitscache) == THRESHOLD_ACTIVE);
                              ...
                              std::vector<uint256>& vHashTxnToUncache, bool fDryRun)
{
    ...
    if (fRequireStandard && tx.nVersion >= 2 && VersionBitsTipState(chainparams.GetConsensus(), Consensus::DEPLOYCLYT_CSV) != THRESHOLD_ACTIVE) {
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params, bool fAssumeMasternodeIsUpgraded)
{
    ...
    for (int i = 0; i < (int)Consensus::MAX_VERSION_BITS_DEPLOYCLYTS; i++) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (VersionBitsState(pindex->pprev, chainparams.GetConsensus(), Consensus::DEPLOYCLYT_CSV, versionbitscache) == THRESHOLD_ACTIVE) {
    ...
    bool fDIP0001Active_context = (VersionBitsState(pindex->pprev, chainparams.GetConsensus(), Consensus::DEPLOYCLYT_DIP0001, versionbitscache) == THRESHOLD_ACTIVE);
    ...
    // CLYCOIN : MODIFIED TO CHECK MASTERNODE PAYCLYTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(CLYCOIN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(CLYCOIN): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // CLYCOIN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(CLYCOIN): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(CLYCOIN): spork is off, skipping transaction locking checks\n");
    }
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    int nLockTimeFlags = 0;
    if (VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYCLYT_CSV, versionbitscache) == THRESHOLD_ACTIVE) {
    ...
    bool fDIP0001Active_context = (VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYCLYT_DIP0001, versionbitscache) == THRESHOLD_ACTIVE);
