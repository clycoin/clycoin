...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete CLYCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no CLYCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old CLYCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected CLYCoin 0.7 datadir."
    ;;
  3)
    echo "Detected CLYCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected CLYCoin 0.8 datadir."
    ;;
esac
