...
class ReplaceByFeeTest(BitcoinTestFramework):
    ...
    def test_doublespend_tree(self):
        ...
        for n in (MAX_REPLACECLYT_LIMIT+1, MAX_REPLACECLYT_LIMIT*2):
            fee = int(0.0001*COIN)
            tx0_outpoint = make_utxo(self.nodes[0], initial_nValue)
            tree_txs = list(branch(tx0_outpoint, initial_nValue, n, fee=fee))
            assert_equal(len(tree_txs), n)
    ...
    def test_too_many_replacements(self):
        ...
        # Try directly replacing more than MAX_REPLACECLYT_LIMIT
        # transactions
        ...
        fee = int(0.0001*COIN)
        split_value = int((initial_nValue-fee)/(MAX_REPLACECLYT_LIMIT+1))
        actual_fee = initial_nValue - split_value*(MAX_REPLACECLYT_LIMIT+1)

        outputs = []
        for i in range(MAX_REPLACECLYT_LIMIT+1):
        ...
        # a higher fee rate
        double_spend_value = (split_value-100*fee)*(MAX_REPLACECLYT_LIMIT+1)
        inputs = []
        for i in range(MAX_REPLACECLYT_LIMIT+1):
